...
// googleSSEToChatSSEPipe 将 Google Gemini SSE 流转换为 Chat Completions SSE 流的 io.ReadCloser
// 通过 io.Pipe 实现异步转换，供 streamChatToMessages 等函数消费
func googleSSEToChatSSEPipe(body io.Reader, model string) io.ReadCloser {
	pr, pw := io.Pipe()
	go func() {
		defer pw.Close()
		...
		for scanner.Scan() {
			...
			if fr, ok := cand["finishReason"].(string); ok && fr != "" {
				lastFinishReason = mapGoogleFinishReason(fr)
			}
			...
			chatPayload, _ := json.Marshal(chatChunk)
		...
		finalPayload, _ := json.Marshal(finalChunk)
		fmt.Fprintf(pw, "data: %s\n\n", finalPayload)
